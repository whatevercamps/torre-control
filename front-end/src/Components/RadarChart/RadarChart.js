...
export default function RadarChart(data, target, options) {
  ...
  const colorScheme = [...d3["schemeAccent"].slice(0, data.length)];

  const cfg = {
    w: (target && Math.min(600, target.offsetWidth)) || 600, //Width of the circle
    h: (target && Math.min(600, target.offsetHeight)) || 600, //Height of the circle
    margin: { top: 0, right: 70, bottom: 130, left: 70 }, //The margins of the SVG
    ...
    color: d3.scaleOrdinal().range(colorScheme), //Color function,
    format: ".2%",
    unit: "",
    legend: false,
  };
  ...
  const allAxis = data[0].axes.map((i, j) => i.axis), //Names of each axis
    ...
    radius = Math.min((cfg.w - cfg.margin.right - cfg.margin.left) / 2, (cfg.h - cfg.margin.top - cfg.margin.bottom) / 2), //Radius of the outermost circle
  ...
  let g = svg
    ...
    .attr(
      "transform",
      "translate(" +
        ((cfg.w - cfg.margin.right - cfg.margin.left) / 2 + cfg.margin.left) +
        "," +
        ((cfg.h - cfg.margin.top - cfg.margin.bottom) / 2 + cfg.margin.top) +
        ")"
    );
  ...
  if (cfg.legend !== false && typeof cfg.legend === "object") {
    ...
    if (cfg.legend.title) {
      legendZone
        ...
        .attr("x", cfg.w - cfg.margin.right - cfg.margin.left - 100)
        .attr("y", 10)
        .attr("font-size", "12px")
        .attr("fill", "#ccc")
    ...
    legend
      ...
      .enter()
      .append("rect")
      .attr("x", cfg.w - cfg.margin.right - cfg.margin.left - 65)
      .attr("y", (d, i) => i * 20)
      .attr("width", 10)
      .attr("height", 10)
      .style("fill", (d, i) => cfg.color(i));
    ...
    legend
      ...
      .enter()
      .append("text")
      .attr("x", cfg.w - cfg.margin.right - cfg.margin.left - 52)
      .attr("y", (d, i) => i * 20 + 9)
      .attr("font-size", "11px")
      .attr("fill", "#aaa")
      .text((d) => d);
